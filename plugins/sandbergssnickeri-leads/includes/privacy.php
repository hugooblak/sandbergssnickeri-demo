...
add_filter(
	...
	function ( $exporters ) {
		$exporters['sandbergssnickeri-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'sandbergssnickeri-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'sandbergssnickeri-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'sandbergssnickeri-leads',
						'group_label' => __( 'Quote requests', 'sandbergssnickeri-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['sandbergssnickeri-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'sandbergssnickeri-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
