...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'sandbergssnickeri-leads' ),
					'singular_name' => __( 'Lead', 'sandbergssnickeri-leads' ),
					'edit_item'     => __( 'Förfrågan', 'sandbergssnickeri-leads' ),
					'search_items'  => __( 'Search leads', 'sandbergssnickeri-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'sandbergssnickeri-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'sandbergssnickeri-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'sandbergssnickeri-leads' ),
			'npl_phone'    => __( 'Phone', 'sandbergssnickeri-leads' ),
			'npl_service'  => __( 'Needs', 'sandbergssnickeri-leads' ),
			'npl_timeline' => __( 'When', 'sandbergssnickeri-leads' ),
			'npl_zip'      => __( 'ZIP', 'sandbergssnickeri-leads' ),
			'npl_source'   => __( 'Source', 'sandbergssnickeri-leads' ),
			'npl_status'   => __( 'Status', 'sandbergssnickeri-leads' ),
			'date'         => __( 'Received', 'sandbergssnickeri-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'sandbergssnickeri-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'sandbergssnickeri-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'sandbergssnickeri-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'sandbergssnickeri-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'sandbergssnickeri-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'sandbergssnickeri-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'sandbergssnickeri-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'sandbergssnickeri-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'sandbergssnickeri-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'sandbergssnickeri-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'sandbergssnickeri-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'sandbergssnickeri-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
