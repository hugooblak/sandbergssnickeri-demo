<?php
/**
 * Sandbergs Snickeri theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Sandbergs Snickeri Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through sa_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'sandbergssnickeri', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'sandbergssnickeri' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'sandbergssnickeri' ) ) );

		register_block_pattern_category( 'sandbergssnickeri-sections', array( 'label' => __( 'Sandbergs Snickeri sections', 'sandbergssnickeri' ) ) );
		register_block_pattern_category( 'sandbergssnickeri-pages', array( 'label' => __( 'Sandbergs Snickeri page layouts', 'sandbergssnickeri' ) ) );
	}
);
