 ...
 * or photo. They call sa_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Sandbergs Snickeri.
 *
 ...
 * @return string
 */
function sa_sample_note() {
	return sa_lead( 'ar_riktig' ) ? '' : ' <span class="sa-sample">(platshållare)</span>';
}
 ...
 * @return string File name for sa_lead_img().
 */
function sa_lead_bild( $i ) {
	$bilder = (array) sa_lead( 'bilder', array() );
 ...
 * @return string
 */
function sa_ring_label() {
	$namn = sa_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, sa_lead( 'telefon' ) ) : 'Ring ' . sa_lead( 'telefon' );
}
